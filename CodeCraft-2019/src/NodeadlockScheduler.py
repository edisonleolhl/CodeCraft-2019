...
class Car(object):
    ...
    def scheduleInit(self, depart_time, route, delay=None):
        if delay:
            self.depart_time = depart_time + delay
        else:
            self.depart_time = depart_time
        self.route, self.routeIndex = route, 0
...
class Road(object):
    ...
    def setBucket(self, cross_id):
        if cross_id == self.to:
            self.provideBucket, self.px, self.py, self.provideNum = self.forwardBucket, self.fx, self.fy, self.forwardNum
            if self.isDuplex:
                self.receiveBucket, self.receiveNum = self.backwardBucket, self.backwardNum
            else:
                self.receiveBucket, self.receiveNum = None, None
        elif cross_id == self.from_:
            self.receiveBucket, self.receiveNum = self.forwardBucket, self.forwardNum
            if self.isDuplex:
                self.provideBucket, self.px, self.py, self.provideNum = self.backwardBucket, self.bx, self.by, self.backwardNum
            else:
                self.provideBucket, self.px, self.py, self.provideNum = None, None, None, None
        else:
            print("Road:%s is not connected CrossId:%s " % (self.id, cross_id))
    ...
    def findSecondPriorityCar(self, first_pri_car_id):
        if self.provideBucket is None:
            print("Please do Car.setBucket() first!")
        self.px, self.py = 0, 0  # if delete, then dead lock, don't know why
        # far_car_list = []
        ...
        for x in range(self.length):
            for y in range(self.channel):
                carId = self.provideBucket[x][y]
                if carId != first_pri_car_id and carId is not None and car_dict[carId].state != 2 and car_dict[carId].priority == 1:
                    car = car_dict[carId]
                    speed = min(car.speed, self.speed)
                    # check current priority car's speed whether it is enough to cross
                    if speed > x:
        ...
        while self.px < self.length:
            carId = self.provideBucket[self.px][self.py]
            if carId != first_pri_car_id and carId is not None and car_dict[carId].state != 2:
                car = car_dict[carId]
                speed = min(car.speed, self.speed)
                # current car's speed is enough and there are no cars in front of it
                if speed > self.px:
    ...
    #
    def setFirstPriorityCarAction(self, action, cross_id):
        if self.provideBucket is None:
            print("Please do Car.setBucket() first!")
    ...
    #
    def receiveCar(self, carId, cross_id):
...
class Cross(object):
    def __init__(self, id, north, east, south, west):
        ...
        self.done = False
        self.update = False
        self.firstPriWaitCar = []
        self.secondPriWaitCar = []
    ...
    def step(self):
        ...
        for presentRoadIndex in range(self.provider.__len__()):
            ...
            while next_car_list[presentRoadIndex] != -1:
                ...
                if present_car.to == self.id:
                    ...
                    provide_car_road.setFirstPriorityCarAction('go', self.id)  # after scheduling first pri car, moveInChannel
                ...
                # move present car to next road(action='go') or to the front grid(action='move')
                    ...
                    action = next_road.receiveCar(present_car.id, self.id)
                    if action == 'wait':
                        # waiting conflict
                        break
                    self.update = True
                    present_car.update(state=2)
                    provide_car_road.setFirstPriorityCarAction(action, self.id)  # after scheduling first pri car, moveInChannel
        ...
        for fromA in range(self.provider.__len__()):
            if next_car_list[fromA] != -1:
                ...
                first_pri_car = next_car_list[fromA]
                self.firstPriWaitCar.append(first_pri_car)
                second_pri_car_id = road_dict[self.provider[fromA]].findSecondPriorityCar(first_pri_car.id)
                if second_pri_car_id != -1:
                    second_pri_car = car_dict[second_pri_car_id]
                    self.secondPriWaitCar.append(second_pri_car)
                else:
                    self.secondPriWaitCar.append(None)
        self.done = done
    ...
    def driveCarInGarage(self):
        ...
        for i in range(readyCars.__len__()):
            ...
            action = road.receiveCar(car_id, self.id)
            if action != 'go':
    ...
    def drivePriCarInGarage(self, to_road=None):
        ...
        for i in range(readyPriCars.__len__()):
            ...
            if road_id not in self.receiver:
                print("Car(%d).Road(%d) not in cross(%d).function:class.outOfCarport" % (car_id, road_id, self.id))
            if to_road is not None and to_road != road_id:
...
class NodeadlockScheduler(object):
    def __init__(self, carInfo, roadInfo, crossInfo, answer_info, preset_answer_info):
        ...
        for line in crossInfo:
            ...
            graph.add_node(int(id), (int(north), int(east), int(south), int(west)))
        ...
        for line in roadInfo:
            ...
            road_dict[int(id)] = Road(int(id), int(length), int(speed), int(channel), int(from_), int(to),
                                      int(isDuplex))
            graph.add_edge(int(from_), int(to), (int(id), int(length), int(speed), int(channel), 0))
            if int(isDuplex) == 1:
                graph.add_edge(int(to), int(from_), (int(id), int(length), int(speed), int(channel), 0))
    ...
    # TODO: change 10% preset route
    def generateAnswer(self):
        answer_list = []
        for car_id, car in car_dict.items():
            if car.preset == 1 and car.presetChanged is False:
                continue
            line = [car_id, car.depart_time]
            line.extend(car.route)
            answer_list.append(line)
        answer_list.sort(key=lambda x:x[1])
        answer_info = []
        for i, li in enumerate(answer_list):
            str_list = [str(x) for x in li]
            st = ",".join(str_list)
            answer_info.append('(' + st + ')')
        return answer_info
    ...
    def initGlobalParams(self):
        ...
        allScheduleTime[0] = 0
        priScheduleTime[0] = 0
        allPriScheduleTime[0] = 0
        car_distribution[0] = 0
        car_distribution[1] = 0
        car_distribution[2] = 0
        remainingPriCnt[0] = self.priCnt
        remainingPriCnt[1] = 1
    ...
    def step(self):
        if TIME[0] % 100 == 0:
            ...
            print("--- cars in garage: %d, on the road: %d, finish trip: %d, remainingPriCnt: %d" % (
                car_distribution[0], car_distribution[1], car_distribution[2], remainingPriCnt[0]))
        for crossId in cross_id_list:
        ...
        while unfinishedCross.__len__() > 0:
            ...
            if self.dead:
                print("WARNNING: present timeslot: [[[%s]]], dead lock in: %s"%(TIME[0], unfinishedCross))
                # print("BREAKING DEADLOCK: postpone unfinishedCross wait cars' depart time && decide new route for wait cars")
                # first_waiting_car = []
                # second_waiting_car = []
                # for cross_id in unfinishedCross:
                #     first_waiting_car += cross_dict[cross_id].firstPriWaitCar # merge list into list
                #     second_waiting_car += cross_dict[cross_id].secondPriWaitCar # merge dict into dict
                # print("FIRST WAITING CAR NUMBER: %s"%first_waiting_car.__len__())
                # print("SECOND WAITING CAR NUMBER: %s"%second_waiting_car.__len__())
                # return first_waiting_car, second_waiting_car
                return True
    ...
    def schedule(self):
        while True:
            ...
            # deadlock !
            if flag:
                return True, TIME[0], None

            # if first_waiting_car:
            #     for road_id, road in road_dict.items():
            #         forward_edge = graph.edge_by_node(road.from_, road.to)
            #         (id, length, speed, channel, num) = graph.edge_data(forward_edge)
            #         graph.update_edge_data(forward_edge, (id, length, speed, channel, road.forwardNum))
            #         if road.isDuplex == 1:
            #             backward_edge = graph.edge_by_node(road.from_, road.to)
            #             (id, length, speed, channel, num) = graph.edge_data(backward_edge)
            #             graph.update_edge_data(forward_edge, (id, length, speed, channel, road.backwardNum))
            #
            #     # TODO: decide new route according to the overall load of the map
            #     for index, first_car in enumerate(first_waiting_car):
            #         if first_car.preset != 1:
            #             path = algo.shortest_path(graph, first_car.from_, first_car.to, usedByScheduler=True)
            ...
            #             first_car.scheduleInit(TIME[0], route)  # postpone wait cars depart_time = TIME[0]
            #         else:
            #             # preset depart time cannot be changed, so change the second priority car
            #             second_car = second_waiting_car[index]
            #             if second_car is not None and second_car.preset != 1:
            #                 path = algo.shortest_path(graph, second_car.from_, second_car.to, usedByScheduler=True)
            #                 route = []  # route = [road_id, road_id, ...]
            #                 for i in range(path.__len__() - 1):
            #                     edge_id = graph.edge_by_node(path[i], path[i + 1])
            #                     road_id = graph.edge_data(edge_id)[0]
            #                     route.append(road_id)
            #                 second_car.scheduleInit(TIME[0], route)  # postpone wait cars depart_time = TIME[0]
            #
            #             # TODO: change at most 10% preset route
            #             if self.changed_cnt < self.max_change:
            #                 path = algo.shortest_path(graph, first_car.from_, first_car.to, usedByScheduler=True)
            #                 route = []  # route = [road_id, road_id, ...]
            #                 for i in range(path.__len__() - 1):
            #                     edge_id = graph.edge_by_node(path[i], path[i + 1])
            #                     road_id = graph.edge_data(edge_id)[0]
            #                     route.append(road_id)
            #                 first_car.scheduleInit(first_car.depart_time, route)  # preset cars depart time cannot be changed
            #                 if first_car.presetChanged is False:
            #                     first_car.presetChanged = True
            #                     self.changed_cnt += 1
            #
            #     print('current preset changed count: %s'%self.changed_cnt)
            #     # init the map
            #     for road_id, road in road_dict.items():
            #         road.init()
            #     for cross_id, cross in cross_dict.items():
            #         cross.init()
            #     for car_id, car in car_dict.items():
            #         car.init()
            #     self.initGlobalParams()
            #     car_distribution[0] = car_dict.__len__()
            #     print("There are %d cars' route preinstalled" % car_dict.__len__())
            #
            #     for carId in car_id_list:
            #         cross_dict[car_dict[carId].from_].garageInit(car_dict[carId].depart_time, carId)
            #
            ...
            if car_distribution[2] == car_id_list.__len__():
                ...
                print('preset changed count: %s'%self.changed_cnt)
                ...
                # answer_info = self.generateAnswer()
                # return TIME[0], answer_info # TODO: answer info
                return False, TIME[0], round(JudgeTime)
            if self.dead:
                break
            TIME[0] += 1
